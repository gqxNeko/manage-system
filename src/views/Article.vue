<template>
  <div class="article">
    ...
    >
      <el-table-column prop="articleName" width="220" size="small" label="文章名">
        <template slot-scope="scope">
          <el-input v-if="scope.row.show" placeholder="请输入修改内容" v-model="scope.row.articleName"></el-input>
          <h2 v-if="!scope.row.show">{{scope.row.articleName}}</h2>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" width="120">
        <template slot-scope="scope">
          <el-button size="small" type="text" @click="handleView()">查看</el-button>
          <el-button
            @click="handleEdit(scope.$index,scope.row)"
            size="small"
            type="text"
          >{{scope.row.show ? "保存":"编辑"}}</el-button>
          <el-button size="small" type="text" @click="deleteRow(scope.$index,tableData)">删除</el-button>
        </template>
      </el-table-column>
